...
New Student accounts provisioned by the Discipline Office also return `onboarding_required` and `onboarding_step`. During the Google step, `google_onboarding_stage` is `EMAIL`, `OTP`, or `OAUTH`, and `onboarding_google_email` contains only the authenticated student's pending address. `POST /api/account/student-onboarding/google-email/request` accepts only `email`; `POST /api/account/student-onboarding/google-email/verify` accepts only `code`. Codes use the shared ten-minute, five-attempt, sixty-second-resend policy and are bound to the destination address. `POST /api/account/student-onboarding/google-link` accepts only `credential` and requires Google's verified email to match the OTP-confirmed address. Only successful binding promotes the address to the recovery email. The profile request remains restricted to academic, contact, and guardian fields. Existing completed and already-linked Student accounts are unaffected.
