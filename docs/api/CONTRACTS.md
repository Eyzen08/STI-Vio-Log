...
`POST /api/students` accepts only `student_number`, required `first_name` and `last_name`, and optional `middle_name` and `suffix`. It rejects email, phone, academic fields, QR values, profile images, ownership fields, and all other unsupported properties. The backend treats the bounded, whitespace-free Student Number as an opaque identifier and local account username, generates the opaque QR value and a random temporary password, and creates the account/profile relationship atomically. The temporary password is returned once in the explicit credential response and is never logged.

`PUT /api/students/:id` allows Admin and Discipline Office users to correct student information with a required audit reason. Changing the Student Number updates the linked local username in the same transaction, invalidates existing sessions, and preserves all disciplinary, DTR, service, message, and clearance relationships. `POST /api/students/:id/password-reset` issues a cryptographically generated temporary password exactly once, activates local access, invalidates existing sessions, and requires a password change at first sign-in. Local credentials and an active Google link are independent login choices for the same student record.
...
New Student accounts provisioned by the Discipline Office also return `onboarding_required` and `onboarding_step`. After password replacement, only `POST /api/account/student-onboarding/google-link` and logout/session maintenance are available until a verified Google account is bound. The binding request accepts only `credential`, derives the Student from the authenticated session, and uses the Google-verified email as the recovery address. The following `POST /api/account/student-onboarding/profile` request accepts only `program`, `section`, `year_level`, `phone_number`, `guardian_name`, `guardian_relationship`, and `guardian_phone_number`. It rejects identity, email, QR, role, state, and other unsupported fields. Successful completion saves academic, contact, guardian, and onboarding state in one transaction and unlocks normal Student APIs. Existing Student accounts default to completed onboarding.
