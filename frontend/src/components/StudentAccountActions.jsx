  ...
  const submit=async(event)=>{
    ...
    try{
      if(mode==='edit'){
        ...
        const response=await fetch(`${API_URL}/api/students/${student.id}`,{method:'PUT',headers,body:JSON.stringify({...edit,phone_number:phone,year_level:edit.year_level===''?null:Number(edit.year_level),reason:why})}),data=await response.json().catch(()=>null)
  ...
  return <div className="student-access-removal">
    ...
    {(mode||secret)&&<Modal title={title} wide={mode==='edit'} dirty={!secret && mode !== 'success' && (Boolean(reason.trim()) || mode === 'edit' && JSON.stringify(edit) !== JSON.stringify(initialEdit(student)))} onClose={() => !busy && close()}>
      {secret?<div className="registration-pending" role="alert"><strong>Copy these credentials now</strong><p>Username: <code>{secret.username}</code></p><p>Temporary password: <code>{secret.password}</code></p><p>The student must change this password after first sign-in.</p><button type="button" onClick={close}>I stored it securely</button></div>:mode==='success'?<div><p className="success-message" role="status">{success}</p><button type="button" onClick={close}>Done</button></div>:<form className="student-form account-action-form" onSubmit={submit}>
        {mode==='edit'&&<div className="student-form-grid"><label>Student Number<input value={edit.student_number} onChange={e=>setEdit({...edit,student_number:digitsOnly(e.target.value)})} inputMode="numeric" pattern={STUDENT_NUMBER_PATTERN} maxLength={11} required/></label><label>First name<input value={edit.first_name} onChange={e=>setEdit({...edit,first_name:capitalizeWords(e.target.value)})} required/></label><label>Middle name<input value={edit.middle_name} onChange={e=>setEdit({...edit,middle_name:capitalizeWords(e.target.value)})}/></label><label>Last name<input value={edit.last_name} onChange={e=>setEdit({...edit,last_name:capitalizeWords(e.target.value)})} required/></label><label>Suffix<input value={edit.suffix} onChange={e=>setEdit({...edit,suffix:capitalizeWords(e.target.value)})}/></label><label>Email<input type="email" value={edit.email} onChange={e=>setEdit({...edit,email:e.target.value})}/></label><PhoneInput id={`student-edit-phone-${student.id}`} name="phone_number" value={edit.phone_number} onChange={e=>setEdit({...edit,phone_number:e.target.value})}/><label>Program<ProgramSelect value={edit.program} onChange={e=>setEdit({...edit,program:e.target.value})}/></label><label>Section<input value={edit.section} onChange={e=>setEdit({...edit,section:e.target.value})}/></label><label>Year level<input type="number" min="1" max="8" value={edit.year_level} onChange={e=>setEdit({...edit,year_level:e.target.value})}/></label></div>}
        {mode==='password'&&<p className="account-action-notice">This creates a one-time temporary password for Student Number <strong>{student.student_number}</strong>, invalidates existing sessions, and requires a password change at first sign-in. Google sign-in remains available.</p>}
        {mode==='google'&&<p className="account-action-notice account-action-notice--danger">The student record and local password access remain preserved. Only the Google link and existing sessions are revoked.</p>}
        <label className="account-reason-field"><span>Required reason</span><textarea value={reason} onChange={e=>setReason(e.target.value)} maxLength="1000" placeholder="Briefly explain why this action is required" required autoFocus={mode!=='edit'}/><small>{reason.length} / 1000</small></label>
