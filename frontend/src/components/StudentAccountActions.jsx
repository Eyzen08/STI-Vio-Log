...
function StudentAccountActions({ token, student, onUpdated }) {
  ...
  const close=()=>{setMode('');setReason('');setError('')}
  ...
  const submit=async(event)=>{
    ...
    try{
      if(mode==='edit'){
        ...
        onUpdated?.(data.student);setSuccess('Student information updated. Existing disciplinary history remains attached.');close()
        ...
        setSecret({username:data.account.username,password:data.temporary_password});setSuccess('Temporary credentials generated.');close()
      }else{
        ...
        setSuccess('Google access removed. The student may link Google again or use issued local credentials.');close()
      }
    }catch(requestError){setError(requestError.message)}finally{setBusy(false)}
  }

  if(secret)return <div className="student-access-removal registration-pending" role="alert"><strong>Copy these credentials now</strong><p>Username: <code>{secret.username}</code></p><p>Temporary password: <code>{secret.password}</code></p><p>The student must change this password after first sign-in.</p><button type="button" onClick={()=>setSecret(null)}>I stored it securely</button></div>
  if(!mode)return <div className="student-access-removal"><div className="registration-review-actions"><button type="button" className="secondary-button" onClick={()=>{setEdit(initialEdit(student));setMode('edit');setSuccess('')}}>Edit information</button><button type="button" className="secondary-button" onClick={()=>{setMode('password');setSuccess('')}}>Issue password</button><button type="button" className="danger-button" onClick={()=>{setMode('google');setSuccess('')}}>Remove Google access</button></div>{success&&<p className="success-message" role="status">{success}</p>}</div>

  return <form className="student-access-removal" onSubmit={submit}>
    <p><strong>{mode==='edit'?'Edit student information':mode==='password'?'Issue temporary Student Account password':'Remove linked Google access'}</strong></p>
