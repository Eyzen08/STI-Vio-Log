    ...
    <div className="certificate-grid">
      <section className="table-card"><div className="table-header"><h3>Student clearance status</h3><span>{visibleStudents.length} of {students.length} students</span></div>
        ...
        <div className="certificate-student-list">{visibleStudents.length ? visibleStudents.map((student) => <article key={student.id} className={`certificate-student-card${selected?.id === student.id ? ' selected' : ''}`}>
          ...
          {student.qualification_status === 'AWAITING_CLEARANCE' && student.clearance_id && <button type="button" className="clearance-approve-button" onClick={() => { setApproveError(''); setApproving(student) }}>Approve Clearance</button>}
