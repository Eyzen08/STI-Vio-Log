...
import { filterDepartmentService, formatLiveServiceTime, isActiveServiceSession, serviceProgress, serviceSessionTiming, summarizeDepartmentService } from '../lib/departmentService.js'
import { formatDuration, formatManilaDateTime } from '../lib/displayFormat.js'

const emptyTimeOut = { attendance_outcome: '', notes: '' }
...
function DepartmentCommunityService({ assignments, loading, error, onOpenScanner, token, onAttendanceUpdated, realtimeSocket }) {
  ...
  const timeOut = async (session) => {
    ...
    if (!form.attendance_outcome) return setActiveError('Select an attendance outcome before time-out.')
    ...
    try {
      const response = await fetch(`${API_URL}/api/community-service/attendance/time-out`, {
        ...
        body: JSON.stringify({ assignment_id: Number(session.assignment_id), student_id: Number(session.student_id), attendance_outcome: form.attendance_outcome, notes: form.notes.trim() })
      })
      const data = await response.json().catch(() => ({}))
  ...
  return <div className="department-service-page">
    ...
    <section className="table-card" aria-busy={activeLoading}>
      ...
      {activeLoading ? <div className="department-empty"><p>Loading active sessions…</p></div> : visibleActiveSessions.length === 0 ? <div className="department-empty"><h4>No students currently timed in</h4><p>Use QR Scan to record a student’s Time-In.</p></div> : <div className="service-assignment-list">{visibleActiveSessions.map((session) => { const form = timeOutForms[session.session_id] || emptyTimeOut; const timing = serviceSessionTiming(session, now); return <article key={session.session_id}><div className="service-assignment-heading"><div><h4>{session.first_name} {session.last_name}</h4><span>{session.student_number} · Assignment #{session.assignment_id}</span></div><strong aria-label="Live elapsed time">{formatLiveServiceTime(timing.elapsedSeconds)}</strong></div>{timing.limitReached&&<p className="timer-limit-notice" role="status">Service limit reached — Time Out required</p>}<p>Timed in: {formatManilaDateTime(session.time_in)}</p><label>Attendance Outcome <small>Required before Time Out</small><select value={form.attendance_outcome} onChange={(event) => updateTimeOut(session.session_id, 'attendance_outcome', event.target.value)} disabled={busySession===session.session_id}><option value="">Select Outcome</option><option value="TODAYS_SERVICE_COMPLETED">Today’s Service Completed</option><option value="LEFT_EARLY">Left Early</option><option value="SERVICE_COMPLETED">Service Completed</option></select></label><label>Service note (optional)<textarea rows="2" maxLength="500" value={form.notes} onChange={(event) => updateTimeOut(session.session_id, 'notes', event.target.value)} disabled={busySession===session.session_id}/></label><button type="button" onClick={() => timeOut(session)} disabled={busySession===session.session_id || !form.attendance_outcome}>{busySession===session.session_id?'Recording…':'Time out and credit hours'}</button></article> })}</div>}
    </section>
