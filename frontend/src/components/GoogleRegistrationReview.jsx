...
function GoogleRegistrationReview({ token, onPendingCountChange, embedded = false }) {
  ...
  const visible=useMemo(()=>registrations.filter((item)=>{
    ...
    return matchesSearch&&(flagFilter==='ALL'||item.review_flag===flagFilter||(flagFilter==='STALE'&&item.is_stale))
  ...
  return <section className="registration-management" aria-label={embedded ? 'Registration review' : undefined} aria-labelledby={embedded ? undefined : 'registration-review-title'}>
    ...
    <section className="table-card registration-directory">
      ...
      <div className="directory-toolbar registration-toolbar"><div className="segmented-control" role="group" aria-label="Registration status">{statuses.map((status)=><button type="button" key={status} className={queueStatus===status?'active':''} aria-pressed={queueStatus===status} onClick={()=>{setSelected(null);setQueueStatus(status)}}>{status[0]+status.slice(1).toLowerCase()}</button>)}</div><input type="search" name="registration-review-filter" autoComplete="off" aria-label="Search registration review" value={search} onChange={(event)=>setSearch(event.target.value)} placeholder="Search name, student number, email, program, or guardian…"/><select aria-label="Filter registration review flags" value={flagFilter} onChange={(event)=>setFlagFilter(event.target.value)}><option value="ALL">All review flags</option><option value="STALE">Stale over 30 days</option><option value="POTENTIAL_DUPLICATE">Potential duplicate</option><option value="MANUAL_RECORD_MATCH">Manual record match</option></select></div>
      {loading?<p className="empty-state" aria-live="polite">Loading registration review…</p>:visible.length===0?<div className="department-empty"><h4>No matching registrations</h4><p>Try another status or filter.</p></div>:<div className="table-wrap"><table className="management-record-table registration-review-table"><thead><tr><th>Student</th><th>Student Number</th><th>Academic Details</th><th>Email Verification</th><th>Google Link</th><th>Review Flag</th><th>Submitted</th><th>Action</th></tr></thead><tbody>{visible.map((item)=><tr key={item.id}><td data-label="Student"><strong>{fullName(item)}</strong><small>{item.google_email||'Email unavailable'}</small></td><td data-label="Student number">{item.student_number}</td><td data-label="Academic details">{item.program||'—'} · Year {item.year_level||'—'} · {item.section||'—'}</td><td data-label="Email verification"><span className="status-badge">Google verified</span></td><td data-label="Google link"><span className="status-badge">{item.google_linked?'Linked':'Not linked'}</span></td><td data-label="Review flag"><span className={`status-badge ${item.review_flag==='POTENTIAL_DUPLICATE'||item.is_stale?'status-open':''}`}>{item.is_stale?'Stale 30+ days':item.review_flag==='POTENTIAL_DUPLICATE'?'Potential duplicate':'Manual match'}</span></td><td data-label="Submitted">{formatManilaDate(item.created_at)}</td><td data-label="Action"><button type="button" className="primary-row-action" onClick={()=>{setError('');setSelected(item)}}>{queueStatus==='PENDING'?'Review':'View history'}</button></td></tr>)}</tbody></table></div>}
    </section>
