...
function AdminDashboard({ students = [], violations = [], assignments = [], pendingRegistrations = 0, unreadMessages = 0, loading, role, onNavigate }) {
  ...
  const timedIn = assignments.filter((item) => item.active_session || item.time_in_at && !item.time_out_at).length
