...
const ActiveSessionTimer = ({ session, now }) => {
  const timing = serviceSessionTiming(session, now)
  return <><time dateTime={`PT${timing.elapsedSeconds}S`}>{formatLiveServiceTime(timing.elapsedSeconds)}</time>{timing.limitReached && <small className="timer-limit-notice">Time Out required</small>}</>
}
