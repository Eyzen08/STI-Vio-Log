  ...
  return <section className="system-dashboard support-access-page">
    ...
    {decision&&<Modal title={`${decision.action==='approve'?'Approve':decision.action==='reject'?'Reject':'Revoke'} request #${decision.request.id}`} onClose={closeDecision} dirty={Boolean(decisionReason)}>
      <form className={`support-decision-dialog support-decision-dialog--${decision.action}`} onSubmit={submitDecision}>
        ...
        <div className="support-decision-actions"><button type="button" className="secondary-button" disabled={busy} onClick={closeDecision}>Cancel</button><button type="submit" className={decision.action==='approve'?'submit-btn':'danger-button'} disabled={busy||decisionReason.trim().length<10}>{busy?'Saving…':decision.action==='approve'?'Approve request':decision.action==='reject'?'Reject request':'Revoke access'}</button></div>
      </form>
    </Modal>}
  </section>
}
