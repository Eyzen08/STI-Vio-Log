...
function MessagesPage({token,role,onUnreadChange,realtimeSocket}){
  ...
  const resetRecipientPicker=()=>{setRecipientSearch('');setRecipientError('');setRecipients([]);setNewForm((current)=>({...current,recipient:''}))}
  const closeNewMessage=()=>{if(creating)return;setShowNew(false);resetRecipientPicker()}
  const loadRecipients=async(searchTerm='')=>{const normalizedSearch=searchTerm.trim();setRecipientLoading(true);setRecipientError('');try{const query=new URLSearchParams();if(normalizedSearch)query.set('search',normalizedSearch);const suffix=normalizedSearch?`?${query}`:'';const response=await fetch(`${API_URL}/api/messages/recipients${suffix}`,{headers:authHeaders}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.message||'Unable to load recipients.');const nextRecipients=Array.isArray(data.recipients)?data.recipients:[];setRecipients(nextRecipients);setNewForm((current)=>nextRecipients.some((item)=>`${item.type}:${item.id??''}`===current.recipient)?current:{...current,recipient:''})}catch(loadError){setRecipients([]);setNewForm((current)=>({...current,recipient:''}));setRecipientError(loadError.message)}finally{setRecipientLoading(false)}}
  const openNewMessage=()=>{resetRecipientPicker();setShowNew(true);loadRecipients()}
  ...
  return <div className={`messages-page messages-inbox${selected?' has-open-thread':''}`}>
    <section className="messages-page-heading"><div><p className="page-breadcrumb">Home / Messages</p><h2>Messages</h2><p>Communicate securely with students and authorized school staff.</p></div><button type="button" className="messages-new-button" onClick={openNewMessage}>+ New Message</button></section>
