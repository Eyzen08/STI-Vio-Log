...
function MessagesPage({token,role,onUnreadChange,realtimeSocket}){
  ...
  return <div className={`messages-page messages-inbox${selected?' has-open-thread':''}`}>
    <section className="messages-page-heading"><div><p className="page-breadcrumb">Home / Messages</p><h2>Messages</h2><p>Communicate securely with students and authorized school staff.</p></div><button type="button" className="messages-new-button" onClick={loadRecipients} disabled={recipientLoading}>{recipientLoading?'Loading…':'+ New Message'}</button></section>
    {error&&<p className="error-message" role="alert">{error}</p>}
    <div className="messages-workspace">
      <section className="conversation-pane" aria-label="Conversations"><div className="conversation-pane-header"><div className="conversation-title-row"><h3>Conversations</h3><span>{conversations.length}{pages>1?' loaded':''}</span></div><label className="conversation-search"><span className="conversation-search-icon" aria-hidden="true">⌕</span><span className="sr-only">Search conversations</span><input type="search" value={search} onChange={(event)=>setSearch(event.target.value)} placeholder="Search names, subjects, or student numbers"/></label><div className="conversation-tabs" role="tablist" aria-label="Conversation filters">{['ALL','UNREAD','CLOSED'].map((value)=><button key={value} type="button" role="tab" aria-selected={tab===value} onClick={()=>setTab(value)}>{value[0]+value.slice(1).toLowerCase()}{value==='UNREAD'&&unreadTotal>0?<b>{unreadTotal}</b>:null}</button>)}</div></div>
        <div className="conversation-list">{loading?<p className="message-state">Loading conversations…</p>:visibleConversations.length===0?<div className="message-state"><strong>No conversations found</strong><span>{search?'Try a different search.':tab==='UNREAD'?'You are all caught up.':'Start a new authorized conversation.'}</span></div>:visibleConversations.map((conversation)=>{const itemParticipant=messageParticipant(conversation,role);const parties=conversationParties(conversation);const unread=Number(conversation.unread_count)>0;return <button type="button" key={conversation.id} className={`${selected?.id===conversation.id?'active ':''}${unread?'unread':''}`.trim()} aria-pressed={selected?.id===conversation.id} aria-label={`${conversation.subject}, conversation between ${parties.student} and ${parties.school}${unread?`, ${conversation.unread_count} unread`:''}`} onClick={()=>open(conversation)}><span className="conversation-avatar" aria-hidden="true">{itemParticipant.name.charAt(0)}</span><span className="conversation-summary"><span className="conversation-subject"><strong>{conversation.subject}</strong><time>{shortDate(conversation.latest_message_at||conversation.updated_at)}</time></span><span className="conversation-parties" title={parties.label}>{parties.label}</span><span className="conversation-preview">{conversation.message_preview||'No messages yet'}</span></span><span className={`conversation-status ${conversation.status.toLowerCase()}`}>{conversation.status}</span>{unread?<b className="conversation-unread" aria-label={`${conversation.unread_count} unread messages`}>{conversation.unread_count}</b>:null}</button>})}{page<pages?<button type="button" className="conversation-load-more" onClick={()=>load({requestedPage:page+1,append:true})}>Load more conversations</button>:null}</div>
      </section>
      <section className="chat-pane" aria-live="polite">{!selected?<div className="message-state message-state--center"><span className="message-empty-icon" aria-hidden="true">✉</span><strong>Select a conversation</strong><span>Choose a thread to read its official message history and reply.</span></div>:<><header className="chat-header" aria-label={`${selected.subject}, conversation between ${selectedParties.student} and ${selectedParties.school}`}><button type="button" className="chat-mobile-back" onClick={()=>setSelected(null)} aria-label="Back to conversations">←</button><span className="conversation-avatar" aria-hidden="true">{participant.name.charAt(0)}</span><div><h3>{selected.subject}</h3><p className="conversation-parties" title={selectedParties.label}>{selectedParties.label}</p></div><span className={`chat-status ${selected.status.toLowerCase()}`}>{selected.status}</span>{canManageStatus?<button type="button" className="chat-status-action" onClick={updateStatus} disabled={statusUpdating}>{statusUpdating?'Saving…':selected.status==='OPEN'?'Close conversation':'Reopen'}</button>:null}</header>
        ...
        <div className="message-record-note"><span aria-hidden="true">◈</span> Messages are retained as official school communication.</div><form className="chat-composer" onSubmit={send}><label><span className="sr-only">Message</span><textarea value={reply} onChange={(event)=>setReply(event.target.value.slice(0,MESSAGE_MAX_LENGTH))} onKeyDown={onComposerKeyDown} maxLength={MESSAGE_MAX_LENGTH} placeholder={selected.status==='OPEN'?'Type a message…':'This conversation is closed. Reopen it to reply.'} disabled={sending||selected.status!=='OPEN'}/><small>{reply.length} / {MESSAGE_MAX_LENGTH}</small></label><button type="submit" disabled={sending||selected.status!=='OPEN'||!reply.trim()}><span>{sending?'Sending…':'Send'}</span><b aria-hidden="true">↗</b></button><p>Enter to send · Shift + Enter for a new line</p></form></>}
      </section>
    </div>
