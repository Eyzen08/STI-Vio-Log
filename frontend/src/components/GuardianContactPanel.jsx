...
function GuardianContactPanel({ token, student, onClose, showClose = true }) {
  ...
  return <section className="table-card guardian-contact-panel" aria-busy={loading}>
    <div className="table-header">
      <div><p className="eyebrow">Private Guardian Contact record</p><h3>Guardian Contact — {studentNumber} · {studentName}</h3></div>
    ...
    {loading && !data ? <p className="empty-state" aria-live="polite">Loading Guardian Contact…</p> : guardians.length === 0 ? <p className="empty-state">No Guardian Contact is recorded.</p> : <>
      <div className="guardian-contact-grid">
        {guardians.map((guardian) => <article key={guardian.id}>
          <div><h4>{guardian.guardian_name}</h4><p>{guardian.relationship || 'Relationship not provided'}{guardian.is_primary ? ' · Primary Guardian Contact' : ''}</p></div>
          <strong>{guardian.phone_number}</strong>
          <div className="guardian-contact-actions"><a href={`tel:${guardian.phone_number}`} aria-label={`Call Guardian Contact ${guardian.guardian_name}`}>Call Guardian Contact</a><a href={`sms:${guardian.phone_number}`} aria-label={`Message Guardian Contact ${guardian.guardian_name}`}>Message Guardian Contact</a></div>
        </article>)}
      </div>
      <form className="student-form guardian-contact-form" onSubmit={submit}>
        <div className="student-form-grid">
          ...
          <label>Contact method<select value={form.method} onChange={(event) => setForm({ ...form, method: event.target.value })}>{CONTACT_METHODS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label>Outcome<select value={form.outcome} onChange={(event) => setForm({ ...form, outcome: event.target.value })}>{CONTACT_OUTCOMES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
    ...
    <p className="scope-note">Guardian Contact information is private. Use it only for authorized student support and discipline follow-up.</p>
  </section>
}
