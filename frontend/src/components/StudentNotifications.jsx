...
function StudentNotifications({ notifications, loading, error }) {
  ...
  return <div className="student-notifications-page">
    ...
    <section className="table-card"><div className="table-header"><div><p className="eyebrow">Newest first</p><h3>Your updates</h3></div><span>{items.length} notifications</span></div>
      {items.length === 0 ? <div className="department-empty"><h4>No notifications yet</h4><p>New account and service updates will appear here.</p></div> : <div className="notification-list">{items.map((item) => <article key={item.id} className={item.is_read ? '' : 'unread'}><span className="notification-indicator" aria-label={item.is_read ? 'Read' : 'Unread'} /><div><div className="notification-heading"><h4>{item.title}</h4><span>{notificationLabel(item.notification_type)}</span></div><p>{item.message}</p><time dateTime={item.created_at}>{notificationDate(item.created_at)}</time></div></article>)}</div>}
    </section><p className="scope-note">Notifications are read-only and belong exclusively to your authenticated account.</p>
  </div>
}
