...
function StudentNotifications({ notifications, loading, error, onMarkRead, onAcknowledge, onMarkAll, onNavigate, audience = 'STUDENT' }) {
  ...
  return <div className="student-notifications-page">
    ...
    <section className="table-card">
      ...
      {visible.length === 0
        ...
        : <div className="notification-list">{visible.map((item) => <article key={item.id} className={item.is_read ? '' : 'unread'}><span className="notification-indicator" aria-label={item.is_read ? 'Read' : 'Unread'} /><div><div className="notification-heading"><h4>{item.title}</h4><span>{notificationLabel(item.category || item.notification_type)}{item.severity && item.severity !== 'INFO' ? ` · ${notificationLabel(item.severity)}` : ''}</span></div><p>{item.message}</p><time dateTime={item.created_at}>{notificationDate(item.created_at)}</time><div className="notification-actions">{item.category === 'SECURITY' && !item.acknowledged_at && onAcknowledge ? <button type="button" className="secondary-button" onClick={() => onAcknowledge(item.id)}>Acknowledge</button> : !item.is_read && onMarkRead && <button type="button" className="secondary-button" onClick={() => onMarkRead(item.id)}>Mark as read</button>}{item.category === 'SECURITY' && item.acknowledged_at && <span className="scope-note">Acknowledged</span>}{item.link_path && onNavigate && <button type="button" className="text-button" onClick={() => onNavigate(item.link_path)}>Open record</button>}</div></div></article>)}</div>}
    </section>
