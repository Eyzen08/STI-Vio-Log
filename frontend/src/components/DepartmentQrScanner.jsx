...
function DepartmentQrScanner({ form, result, error, verifiedQr, isScanning, isSubmitting, departments=[], recorder, recentScans=[], onFieldChange, onStartCamera, onStopCamera, onSwitchCamera, onAction }) {
  ...
  return <section className="qr-attendance" aria-labelledby="qr-attendance-title">
    ...
    <article className="qr-stage-card record-stage"><h3><b>3</b> Record Attendance</h3><div className={`record-fields ${departmentLocked?'record-fields--scoped':'record-fields--admin'}`}>
      ...
      <label><span className="record-field-label">Status <small>Required before Time Out</small></span><select name="condition" value={form.condition||''} onChange={onFieldChange} disabled={isSubmitting}><option value="">Select Status</option><option value="SATISFACTORY">Completed</option><option value="NEEDS_FOLLOW_UP">Needs Action</option><option value="INCIDENT_REPORTED">Reported</option></select></label>
    </div><div className="attendance-footer"><p><strong>Recorded as {recorder?.username||'authenticated staff'}</strong><span>{roleLabel(recorder?.role)} · Recorder and timestamps are set by the server.</span></p><div><button type="button" onClick={()=>onAction('time-in')} disabled={!verified||isSubmitting||state.active||!form.supervising_officer_id||noOfficer}>{isSubmitting?'Recording...':'Time In'}</button><button type="button" className="time-out-button" onClick={()=>onAction('time-out')} disabled={!verified||isSubmitting||!state.active||!form.condition||!form.supervising_officer_id||noOfficer}>{isSubmitting?'Recording...':'Time Out & Credit Hours'}</button></div></div>
