...
function StudentDashboard({ profile, violations = [], assignments = [], clearanceRecords = [], eligibility, loading, error, onNavigate }) {
  ...
  return <div className="student-dashboard portal-dashboard">
    ...
    <section className="dashboard-card dashboard-table-card">
      ...
      {violations.length === 0 ? <p className="empty-state">No violations on record.</p> : <div className="table-wrap"><table><thead><tr><th>Date</th><th>Offense</th><th>Type</th><th>Status</th><th>Required service</th></tr></thead><tbody>{violations.slice(0,5).map((violation)=><tr key={violation.id}><td>{formatIncidentDateTime(violation.incident_date, violation.incident_time)}</td><td><strong>{violation.exact_offense || violation.violation_type_name || `Violation #${violation.id}`}</strong></td><td><OffenseIndicator level={violation.offense_indicator_level || offenseLevel} compact/></td><td><span className={`status-badge status-${String(violation.status).toLowerCase()}`}>{violation.status}</span></td><td>{formatDuration(violation.required_service_hours)}</td></tr>)}</tbody></table></div>}
    </section>
  </div>
}
