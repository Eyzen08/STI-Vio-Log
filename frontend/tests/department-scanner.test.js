...
test('all authorized staff roles use the single three-stage attendance workspace', async () => {
  ...
  assert.match(scannerSource, /!departmentLocked&&<label><span className="record-field-label">Department/)
  assert.match(scannerSource, /className="record-field-label">Supervising officer/)
  assert.match(scannerSource, /record-fields--scoped/)
  assert.match(scannerSource, /record-fields--admin/)
  assert.match(scannerSource, /Attendance Outcome <small>Required before Time Out<\/small>/)
  assert.match(scannerSource, /<option value="">Select Outcome<\/option>/)
  assert.match(scannerSource, /<option value="TODAYS_SERVICE_COMPLETED">Today’s Service Completed<\/option>/)
  assert.match(scannerSource, /<option value="LEFT_EARLY">Left Early<\/option>/)
  assert.match(scannerSource, /<option value="SERVICE_COMPLETED">Service Completed<\/option>/)
  assert.doesNotMatch(scannerSource, /SATISFACTORY|NEEDS_FOLLOW_UP|INCIDENT_REPORTED/)
