...
test('support access decisions use an accessible contextual dialog', () => {
  const panel = read('src/components/SupportAccessPanel.jsx')
  const css = read('src/App.css')
  assert.doesNotMatch(panel, /window\.prompt/)
  assert.match(panel, /<Modal title=/)
  assert.match(panel, /Original request/)
  assert.match(panel, /Decision reason/)
  assert.match(panel, /minLength="10" maxLength="1000"/)
  assert.match(panel, /requested_scopes\?\.map\(scopeLabel\)/)
  assert.match(panel, /Approve request/)
  assert.match(panel, /Reject request/)
  assert.match(panel, /Revoke access/)
  assert.match(css, /\.support-decision-summary \{[^}]*grid-template-columns: 1fr 1fr;/s)
  assert.match(css, /@media \(max-width: 600px\)[\s\S]*?\.support-decision-summary \{ grid-template-columns: 1fr; \}/)
})
