...
test('each supported role receives its own dashboard and navigation', () => {
  ...
  assert.deepEqual(getNavItems('DEPARTMENT_HEAD').map(({ label }) => label), ['QR Scan', 'Service Results'])
})
...
test('protected routes permit only their declared roles', () => {
  ...
  assert.equal(resolveRoute('/admin/department-accounts', 'ADMIN').status, 'allowed')
  assert.equal(resolveRoute('/admin/department-accounts', 'DISCIPLINE_OFFICE').status, 'allowed')
  assert.equal(resolveRoute('/admin/accounts', 'ADMIN').status, 'not_found')
  assert.equal(resolveRoute('/admin/accounts', 'DISCIPLINE_OFFICE').status, 'not_found')
