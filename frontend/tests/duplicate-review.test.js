...
test('duplicate review legacy route is admin-only and opens the combined workspace',()=>{const route=APP_ROUTES.find(item=>item.path==='/admin/duplicate-review');assert.deepEqual(route.roles,['ADMIN']);assert.equal(route.navigation,false);assert.equal(resolveRoute(route.path,'ADMIN').redirectTo,'/admin/registrations');assert.equal(resolveRoute(route.path,'DISCIPLINE_OFFICE').status,'unauthorized')})
