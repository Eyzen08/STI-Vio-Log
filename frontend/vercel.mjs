...
export const buildConfig = (env = process.env) => {
  ...
  const apiOrigin = env.API_PROXY_ORIGIN ? normalizeOrigin(env.API_PROXY_ORIGIN, 'API_PROXY_ORIGIN') : null
  if (target === 'production' && !apiOrigin) throw new Error('API_PROXY_ORIGIN is required for production deployments')
