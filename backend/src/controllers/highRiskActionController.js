...
module.exports={
  ...
  executeDirect:async(req,res)=>{try{assertAllowedFields(req.body,['action_type','target_id','reason','step_up_token']);return res.json({success:true,...await service.executeDirect({requesterId:req.user.id,actionType:req.body.action_type,targetId:req.body.target_id,reason:req.body.reason,stepUpToken:req.body.step_up_token})})}catch(error){return fail(res,error)}}
};
