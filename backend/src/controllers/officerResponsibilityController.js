...
const createOfficerResponsibilityController = ({ service = createOfficerResponsibilityService({ pool }) } = {}) => {
  ...
  const available = async (req, res) => { try { assertAllowedFields(req.query, ['department_id']); const departmentId = req.user.role === 'ADMIN' ? req.query.department_id : req.user.department_id; return res.json({ success: true, officers: await service.available({ departmentId }) }); } catch (error) { return fail(res, error); } };
