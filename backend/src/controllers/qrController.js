...
const findStudentAndAssignment = async (qrCode) => {
    ...
    const assignmentResult = await pool.query(
        `SELECT a.* FROM community_service_assignments a JOIN violations v ON v.id = a.violation_id
         ...
         ORDER BY a.id DESC LIMIT 1`, [student.id]);
...
const respondError = (res, error, operation) => {
    ...
    const status = error instanceof CommunityServiceSessionError ? error.statusCode : 500;
...
const scanQrCode = async (req, res) => {
    ...
    try { return res.json({ success: true, message: "QR code scanned successfully", ...(await findStudentAndAssignment(req.body.qr_code)) }); }
...
const handle = (operation) => async (req, res) => {
    if (!req.body.qr_code || !req.staffDepartmentId) return res.status(400).json({ success: false, message: "qr_code and a valid staff department are required" });
    try {
        const { student, assignment } = await findStudentAndAssignment(req.body.qr_code);
