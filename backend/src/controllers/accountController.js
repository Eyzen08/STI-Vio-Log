...
const createStudentOnboardingController=({googleService=null,onboardingService=createStudentOnboardingService({pool,otpService:createOtpService({pool,sendOtp:createEmailService().sendOtp})})}={})=>{
  ...
  return {
  requestGoogleEmail:async(req,res)=>{try{assertAllowedFields(req.body,['email']);if(req.user.role!=='STUDENT')return sendError(res,403,'FORBIDDEN','Student account required');const state=await onboardingService.requestGoogleEmail({userId:req.user.id,email:req.body?.email});return res.json({success:true,message:'Verification code sent',user:sessions.publicUser({...req.user,...state,pending_google_email:state.onboarding_google_email,pending_google_email_verified_at:null})});}catch(error){return sendError(res,error.statusCode||500,error.code||'INTERNAL_ERROR',error.statusCode?error.message:'Unable to send verification code')}},
  verifyGoogleEmail:async(req,res)=>{try{assertAllowedFields(req.body,['code']);if(req.user.role!=='STUDENT')return sendError(res,403,'FORBIDDEN','Student account required');const state=await onboardingService.verifyGoogleEmail({userId:req.user.id,code:req.body?.code,ipAddress:req.ip||null});return res.json({success:true,message:'Email verified',user:sessions.publicUser({...req.user,...state,pending_google_email:state.onboarding_google_email,pending_google_email_verified_at:new Date()})});}catch(error){return sendError(res,error.statusCode||500,error.code||'INTERNAL_ERROR',error.statusCode?error.message:'Unable to verify email')}},
  ...
  completeOnboarding:async(req,res)=>{try{assertAllowedFields(req.body,['program','section','year_level','phone_number','guardian_name','guardian_relationship','guardian_phone_number']);if(req.user.role!=='STUDENT')return sendError(res,403,'FORBIDDEN','Student account required');const state=await onboardingService.completeProfile({userId:req.user.id,program:req.body?.program,section:req.body?.section,yearLevel:req.body?.year_level,phoneNumber:req.body?.phone_number,guardianName:req.body?.guardian_name,guardianRelationship:req.body?.guardian_relationship,guardianPhoneNumber:req.body?.guardian_phone_number,ipAddress:req.ip||null});return res.json({success:true,message:'Student onboarding completed',user:sessions.publicUser({...req.user,must_change_password:false,...state,onboarding_required:false,onboarding_completed_at:new Date()})});}catch(error){return sendError(res,error.statusCode||500,error.code||'INTERNAL_ERROR',error.statusCode?error.message:'Unable to complete student onboarding')}}
  };
};
