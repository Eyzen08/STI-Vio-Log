...
const createAdminAccountService = ({ pool, otpService } = {}) => {
  ...
  const getProfile = async ({ userId }) => {
    ...
    const row = (await pool.query(`SELECT u.id,u.username,u.role,ap.first_name,ap.last_name,ap.email,ap.email_verified FROM users u LEFT JOIN admin_profiles ap ON ap.user_id=u.id WHERE u.id=$1 AND u.role='DISCIPLINE_ADMIN'`, [Number(userId)])).rows[0];
  ...
  const updateProfile = async ({ userId, firstName, lastName, username, email, ipAddress = null }) => {
    ...
    try {
      ...
      const current = (await client.query(`SELECT u.id,u.username,ap.email FROM users u LEFT JOIN admin_profiles ap ON ap.user_id=u.id WHERE u.id=$1 AND u.role='DISCIPLINE_ADMIN' FOR UPDATE OF u`, [Number(userId)])).rows[0];
  ...
  const verifyEmail = async ({ userId, code, ipAddress = null }) => {
    ...
    try {
      await client.query('BEGIN');
      const account = (await client.query(`SELECT u.id FROM users u JOIN admin_profiles ap ON ap.user_id=u.id WHERE u.id=$1 AND u.role='DISCIPLINE_ADMIN' AND ap.email IS NOT NULL FOR UPDATE OF u`, [Number(userId)])).rows[0];
