  ...
  const requestPasswordReset = async ({ identifier }) => {
    ...
    const account = (await pool.query(
      `SELECT u.id,COALESCE(s.email,ap.email) email,
              CASE WHEN u.role='DISCIPLINE_ADMIN' THEN 'ADMIN_PASSWORD_RESET' ELSE 'STUDENT_PASSWORD_RESET' END purpose
       ...
       WHERE u.is_active=TRUE AND ((u.role='STUDENT' AND u.email_verified=TRUE AND (LOWER(u.username)=LOWER($1) OR LOWER(s.email)=LOWER($1)))
          OR (u.role='DISCIPLINE_ADMIN' AND ap.email_verified=TRUE AND (LOWER(u.username)=LOWER($1) OR LOWER(ap.email)=LOWER($1)))) LIMIT 1`,
      [value]
    )).rows[0];
    if (account?.email) {
      try {
  ...
  const resetPassword = async ({ resetToken, newPassword, confirmPassword, ipAddress = null }) => {
    ...
    try {
      ...
      await client.query(
        ...
        [authorization.user_id, user.role==='DISCIPLINE_ADMIN'?'ADMIN_PASSWORD_RESET':'STUDENT_PASSWORD_RESET', user.role==='DISCIPLINE_ADMIN'?'Administrator completed verified password reset':'Student completed verified password reset', ipAddress]
      );
      await client.query('COMMIT');
    } catch (error) {
      try { await client.query('ROLLBACK'); } catch (_) {}
