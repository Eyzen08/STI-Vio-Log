...
const ADMIN_ROLES = new Set(['DISCIPLINE_ADMIN']);
...
const createSystemAccountSecurityService = ({ pool, hashPassword=(value)=>bcrypt.hash(value,12), randomBytes=crypto.randomBytes }={}) => {
  ...
  const lock = async ({ actorId, targetId, reason, expectedVersion }) => {
  ...
  const initiateRecovery = async ({ actorId, targetId, reason, expectedVersion }) => {
    const why=cleanReason(reason);
