...
test('violations Excel export mirrors the readable generated report layout', async () => {
  ...
  assert.equal(sheet.getCell('D2').value, 'Major Offense - Category A')
  ...
  assert.equal(sheet.getCell('A1').fill, undefined)
  assert.equal(sheet.getCell('A2').fill, undefined)
  assert.equal(sheet.getCell('A1').border, undefined)
  assert.equal(sheet.getCell('A2').border, undefined)
