...
test('unified department creation derives actor identity and accepts only designed fields',async()=>{let received;const controller=createAccountAdministrationController({service:{async createDepartmentOfficer(input){received=input;return{account:{id:9},department:{id:3},temporary_password:'once'}}}});const res=response();await controller.createDepartmentOfficer({user:{id:4},body:{department_name:'Library',username:'library.head',role:'DEPARTMENT_HEAD',first_name:'A',last_name:'B'}},res);assert.equal(res.statusCode,201);assert.equal(received.actorId,4);assert.equal(received.departmentName,'Library');assert.equal(res.body.temporary_password,'once')});
