...
test('Google identity linking normalizes names, commits link and audit, then issues a session', async () => {
  ...
  assert.deepEqual(result, { token: 'token-44', user: { id: 44, username: 'student44', role: 'STUDENT', first_name: 'Maria Ana', last_name: 'De Leon', full_name: 'Maria Ana De Leon', password_change_required:false, onboarding_required:false, onboarding_step:'COMPLETE' } });
...
test('linked Google login updates metadata and audit atomically', async () => {
  ...
  assert.deepEqual(await service.loginStudent({ credential: 'token' }), { token: 'session-token', user: { id: 44, username: 'student44', role: 'STUDENT', first_name: 'Maria', last_name: 'Santos', full_name: 'Maria Santos', password_change_required:false, onboarding_required:false, onboarding_step:'COMPLETE' } });
