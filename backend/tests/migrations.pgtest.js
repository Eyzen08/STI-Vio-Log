...
test("fresh migration chain is complete and idempotent", async () => {
    ...
    try {
        ...
        assert.deepEqual(first.applied, ["001_initial_schema.sql", "002_violation_lifecycle.sql", "003_service_clearance_sync.sql", "004_community_service_sessions.sql", "005_google_identity_links.sql", "006_google_student_registrations.sql", "007_google_department_registrations.sql", "008_account_security.sql", "009_staff_profiles.sql", "010_handbook_violation_categories.sql", "011_student_registration_profile.sql", "012_parent_contact_logs.sql", "013_service_assignment_department.sql"]);
        ...
        for (const name of ["users", "students", "violations", "violation_actions", "community_service_assignments", "community_service_sessions", "community_service_progress_history", "student_clearance", "audit_logs", "google_identity_links", "google_student_registrations", "google_department_registrations", "staff_profiles", "parent_contact_logs", "schema_migrations"]) assert.ok(tables.includes(name), `missing ${name}`);
