...
test("fresh migration chain is complete and idempotent", async () => {
    ...
    try {
        ...
        for (const name of ["users", "students", "violations", "violation_actions", "student_offense_escalations", "community_service_assignments", "community_service_sessions", "community_service_session_officer_history", "community_service_progress_history", "student_clearance", "audit_logs", "google_identity_links", "google_student_registrations", "google_department_registrations", "staff_profiles", "parent_contact_logs", "discipline_officer_signatures", "clearance_certificates", "clearance_certificate_signatures", "officer_availability", "officer_department_assignments", "schema_migrations"]) assert.ok(tables.includes(name), `missing ${name}`);
        ...
        assert.equal(attempts.filter((item) => item.status === "rejected" && item.reason.code === "23505").length, 1);
        const linkedUserId = (await pool.query("SELECT user_id FROM google_identity_links WHERE google_subject = 'google-subject-one'")).rows[0].user_id;
        await assert.rejects(
            pool.query("INSERT INTO google_identity_links (user_id, google_subject) VALUES ($1, 'google-subject-two')", [linkedUserId]),
        ...
        const listedStaff=await accounts.list({search:'discipline.two'});assert.equal(listedStaff.accounts.length,1);assert.equal('temporary_password' in listedStaff.accounts[0],false);
...
test("production-shaped legacy upgrade preserves events and canonicalizes statuses", async () => {
    ...
    try {
        ...
        assert.deepEqual(legacyResult.applied, ["002_violation_lifecycle.sql", "003_service_clearance_sync.sql", "004_community_service_sessions.sql", "005_google_identity_links.sql", "006_google_student_registrations.sql", "007_google_department_registrations.sql", "008_account_security.sql", "009_staff_profiles.sql", "010_handbook_violation_categories.sql", "011_student_registration_profile.sql", "012_parent_contact_logs.sql", "013_service_assignment_department.sql", "014_event_notifications.sql", "015_enrollment_verification.sql", "016_secure_messaging.sql", "017_discipline_student_record_review.sql", "018_service_result_review.sql", "019_student_password_auth.sql", "020_student_password_registration_profile.sql", "021_message_department_scope.sql", "022_unified_department_officers.sql", "023_admin_account_profiles.sql", "024_clearance_certificates.sql", "025_offense_escalation_incident_time.sql", "026_officer_responsibility.sql", "027_google_registration_identity_names.sql", "028_administrator_role_separation.sql", "029_convert_legacy_administrators.sql", "030_temporary_support_access.sql"]);
