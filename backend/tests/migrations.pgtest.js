...
test("fresh migration chain is complete and idempotent", async () => {
    ...
    try {
        ...
        for (const name of ["users", "students", "violations", "violation_actions", "student_offense_escalations", "community_service_assignments", "community_service_sessions", "community_service_progress_history", "student_clearance", "audit_logs", "google_identity_links", "google_student_registrations", "google_department_registrations", "staff_profiles", "parent_contact_logs", "discipline_officer_signatures", "clearance_certificates", "clearance_certificate_signatures", "schema_migrations"]) assert.ok(tables.includes(name), `missing ${name}`);
